...
//	#define Error()   do{LATDbits.LATD0 = 1; LATDbits.LATD1 = 1; LATDbits.LATD2 = 1;} while(0);
 #define mLED              LATBbits.LATB13    //LED1-->Blink
	// Blinks LED 1 on USB Starter kit
	#define BlinkLED() (mLED = ((ReadCoreTimer() & 0x0800000) != 0))
	#define InitLED() do{	\
                                                        PORTSetPinsDigitalOut(IOPORT_B, BIT_7 | BIT_13); \
							PORTSetBits(IOPORT_B, BIT_7 | BIT_13);\
						}while(0)

   // Error indication.
	#define Error()   do{PORTClearBits(IOPORT_B, BIT_7 | BIT_13);} while(0);
